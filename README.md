...
```bash
sudo bash deploy/debian.sh install
```
...
```bash
sudo bash deploy/debian.sh install-cfst
```

常用命令：

```bash
sudo bash deploy/debian.sh update
sudo bash deploy/debian.sh restart
bash deploy/debian.sh status
bash deploy/debian.sh logs
```

自定义端口或目录：

```bash
sudo PORT=8080 APP_DIR=/opt/atuo-cf bash deploy/debian.sh install
```
